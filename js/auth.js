...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!users.find(user => user.email === '1')) {
        users.push({ username: 'Yönetici', email: '1', phone: '+901', password: '1', description: '' });
    }
...
function registerUser(username, identifier, phone, password, type = 'email') {
    ...
    users.push({ username, email: type === 'email' ? identifier : '', phone: type === 'phone' ? identifier : phone, password, description: '' });
    localStorage.setItem('users', JSON.stringify(users));
    return true;
}
